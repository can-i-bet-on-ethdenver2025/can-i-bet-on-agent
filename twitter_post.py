...
def post_tweet(access_token, message, in_reply_to_id=None):
    ...
    response = requests.post(
        "https://api.twitter.com/2/tweets", headers=headers, json=data
    )
...
def refresh_and_store_tokens(redis_client):
    ...
    redis_client.set(
        "TWITTER_REFRESH_TOKEN", new_tokens["refresh_token"], ex=expiration
    )
...
def post_tweet_using_redis_token(tweet_text, in_reply_to_id=None):
    try:
        print("Posting tweet using Redis token...")
        # Get Redis client
        redis_client = get_redis_client()
        print("tweet text: ", tweet_text)
        print("in_reply_to_id: ", in_reply_to_id)
        ...
        try:
            ...
            result = post_tweet(access_token, tweet_text, in_reply_to_id)
            print(
                f"Successfully posted tweet after refreshing tokens! Tweet ID: {result['data']['id']}"
            )
            return result["data"]["id"]

        except Exception as refresh_error:
            print(
                f"Error refreshing tokens or posting tweet after refresh: {str(refresh_error)}"
            )
            return None
...
def main():
    try:
        ...
        expiration = int(timedelta(days=90).total_seconds())
        redis_client.set(
            "TWITTER_ACCESS_TOKEN", new_tokens["access_token"], ex=expiration
        )
        redis_client.set(
            "TWITTER_REFRESH_TOKEN", new_tokens["refresh_token"], ex=expiration
        )
