...
def get_pool_id_hex(pool_id):
    """
    Converts a pool ID integer to its hexadecimal representation.

    Args:
        pool_id (int): The pool ID as an integer
        add_suffix (bool, optional): Whether to add '00' suffix to the hex. Defaults to False.

    Returns:
        str: The hexadecimal representation of the pool ID with '0x' prefix
    """
    # Convert to hex, remove '0x' prefix
    hex_without_prefix = hex(pool_id)[2:]

    # Ensure even length with zero padding
    if len(hex_without_prefix) % 2 != 0:
        hex_without_prefix = "0" + hex_without_prefix

    hex_without_prefix = hex_without_prefix + "00"
...
def call_grade_pool_contract(pool_id):
    try:
        # Build the transaction
        tx = CONTRACT.functions.gradeBet(pool_id).build_transaction(
            {
                "from": ACCOUNT.address,
                "nonce": w3.eth.get_transaction_count(ACCOUNT.address),
                "gas": GAS_LIMIT,
